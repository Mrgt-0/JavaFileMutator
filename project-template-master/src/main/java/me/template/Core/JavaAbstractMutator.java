...
public abstract class JavaAbstractMutator {
    ...
    private void collectImports(CtType<?> type, java.util.Set<String> imports) {
        ...
        for (spoon.reflect.reference.CtTypeReference<?> ref : references) {
            ...
            if (qName == null || qName.isEmpty() || qName.equals("nulltype") || qName.equals("<nulltype>")) continue;
            if (ref.isPrimitive() || ref.isArray()) continue;
            if (qName.startsWith("java.lang.")) continue;
            if (qName.startsWith("java.") && !qName.contains(".")) continue;
            imports.add(qName);
        }
    }
